...
# %%
carpeta = 'C:/Users/DAFNE/OneDrive/Documentos/TP2Labo/'
...
df_label = df[['label']] #y del modelo
...
while(i > 0):
    ...
    X_dev = X_dev[dif.columns]
    X_eval = X_eval[dif.columns]
    
    for k in range(1,11):
            # Declaramos el tipo de modelo
            neigh = KNeighborsClassifier(n_neighbors=k) #KNN con k = 3
            # Entrenamos el modelo
            neigh.fit(X_dev, Y_dev.values.ravel())
            score_pixeles_x_vecinos[i-1, k-1] = neigh.score(X_eval, Y_eval)
    i = i -1
